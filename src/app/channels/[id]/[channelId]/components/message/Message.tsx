...
export default function Message({ message, userData, setReplyTo, setIsReplying, isReplied, messages }: {
    ...
    return (
        <div id={message.id} className={cn(`hover:bg-[#2E3035] w-full group ${!message?.isGrouped && "my-2"} flex items-center`)}>
            ...
            <div className="px-[1rem] w-full relative pr-10">
                ...
                <div className={cn(`text-[#DBDEE1] w-full relative ${message?.isGrouped && "px-[3.7rem] py-1"} ${message.isLoading && "text-[#6C6E73]"}`)}>
                    {message.replyTo && <RepliedMessage message={repliedMessage} users={userData} />}
                    {message.isGrouped && <span className="text-[#949BA4] mx-2 text-[0.7rem] absolute left-[-10px] bottom-[6px] hidden group-hover:block  cursor-default select-none">{messageDate}</span>}{message?.title} <div className={cn(`w-fit h-fit absolute right-0 ${message.isGrouped ? "top-[-6px]" : "top-[-30px]"} -translate-y-1/2 bg-[#313338] border border-[#27292D] rounded-[4px] overflow-hidden hidden  items-center  group-hover:flex cursor-pointer z-50`)}>
